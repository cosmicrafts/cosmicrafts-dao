...
struct Player {
id: u64, // Principal or some other unique identifier
name: String,
}
...
planet_type: String,
moons: Vec<Moon>,
owner_id: Option<u64>, // This should be the principal
...
for name in planet_names {
    let planet = create_planet(name.to_string());
...
#[update]
fn register_player(name: String) -> u64 {
let player_id = NEXT_PLAYER_ID.with(|id| {
...
PLAYERS.with(|players| {
    players.borrow_mut().insert(player_id, player);
});
...
#[query]
fn get_player(player_id: u64) -> Option<Player> {
PLAYERS.with(|players| players.borrow().get(&player_id).cloned())
}
...
// --- Planet Management ---
#[update]
fn create_planet(name: String) -> Planet {
...
// Correct initialization of `Planet`
let planet = Planet {
    id: planet_id,
    name,
    planet_type: "Terrestrial".to_string(), // Add missing field
    moons: vec![], // Add missing field
    owner_id: None,
...
#[update]
fn claim_planet(player_id: u64, planet_id: u64) -> Result<(), String> {
PLANETS.with(|planets| {
    let mut planets = planets.borrow_mut();
    if let Some(planet) = planets.get_mut(&planet_id) {
        if planet.owner_id.is_none() {
            planet.owner_id = Some(player_id);
            Ok(())
        } else {
...
// --- Building Management ---
#[update]
fn build_structure(player_id: u64, planet_id: u64, building_type: BuildingType) -> Result<u64, String> {
// Check if the player owns the planet
let is_owner = PLANETS.with(|planets| {
    planets.borrow().get(&planet_id).map(|planet| planet.owner_id) == Some(Some(player_id))
});
...
#[update]
fn upgrade_building(player_id: u64, planet_id: u64, building_id: u64) -> Result<(), String> {
// Check if the player owns the planet
let is_owner = PLANETS.with(|planets| {
    planets.borrow().get(&planet_id).map(|planet| planet.owner_id) == Some(Some(player_id))
});
...
#[update]
fn create_fleet(player_id: u64) -> Result<u64, String> {
// Check if the player exists
let player_exists = PLAYERS.with(|players| players.borrow().contains_key(&player_id));
if !player_exists {
...
#[update]
fn build_ship(player_id: u64, planet_id: u64, ship_type: ShipType) -> Result<u64, String> {
...
// (Assuming the player has only one fleet for simplicity)
let fleet_id_opt = FLEETS.with(|fleets| {
    let mut fleets = fleets.borrow_mut();
    if let Some(fleet) = fleets.values_mut().find(|f| {
        // Check if the fleet belongs to the player
        // (Assuming the fleet is associated with the player)
        true // Placeholder logic
    }) {
        fleet.ships.push(new_ship);
        Ok(ship_id)
    } else {
        Err("Fleet not found for the player.".to_string())
    }
});
